...
function makeImg(panel, idx, onLoad, onError) {
  ...
  img.alt   = `Scene ${idx + 1}`;
  img.className = "w-full h-full object-contain bg-black/40";
  img.onload  = onLoad;
  img.onerror = onError;
...
function buildSlideshow() {
  ...
  panels.forEach((panel, idx) => {
    ...
    const img = makeImg(
      ...
      idx,
      () => {
        console.log(`[SCENOVA] Scene ${idx + 1} loaded successfully: ${img.src.substring(0,60)}...`);
        if (slide.contains(skel)) slide.removeChild(skel);
        slide.appendChild(img);
      },
      () => {
        console.error(`[SCENOVA] Scene ${idx + 1} FAILED to load. URL Length: ${img.src.length}`);
